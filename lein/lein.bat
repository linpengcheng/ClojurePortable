...
rem ==================USERPROFILE==================
set USERPROFILE=%CLJBOX_HOME%portable
if not exist %USERPROFILE%                  mkdir %USERPROFILE%
if not exist %USERPROFILE%\.m2              mkdir %USERPROFILE%\.m2
if not exist %USERPROFILE%\.m2\repository   mkdir %USERPROFILE%\.m2\repository
...
rem ==================Maven==================
if "x%M2_HOME%" == "x" (
    set  M2_HOME=%CLJBOX_HOME%Maven\maven3
)
if "x%M2_REPO%" == "x" (
    set  M2_REPO=%CLJBOX_HOME%Maven\.m2\repository
)
set      mvn=%CLJBOX_HOME%Maven
set mvn_repo=%M2_REPO%
set mvn_home=%M2_HOME%
set  mvn_bin=%mvn_home%\bin
set mvn_conf=%mvn_home%\conf
set       M2=%mvn_bin%
rem ==================JRI==================
rem if "x%R_HOME%" == "x" (
    set R_HOME=%CLJBOX_HOME%R\R3
rem )
set LD_LIBRARY_PATH=%R_HOME%\library
set           R_BIN=%R_HOME%\bin\i386
set         JRI_DIR=%R_HOME%\library\rJava\jri\i386
set     RStudio_DIR=%CLJBOX_HOME%R\RStudio
set     Rtools_Home=%CLJBOX_HOME%R\Rtools
rem ==================ConEmu==================
set CONEMU_HOME=%CLJBOX_HOME%ConEmu
rem ==================git==================
set GIT_HOME=%CLJBOX_HOME%git
set GIT_BIN=%GIT_HOME%\bin
set GIT_CMD=%GIT_HOME%\cmd
set GIT_MINGW32_BIN=%GIT_HOME%\mingw32\bin
set GIT_USER_BIN=%GIT_HOME%\usr\bin
rem ========== lein & JDK =======================
if "x%LEIN_HOME%" == "x" (
    set LEIN_HOME=%~dp0
)
...
set LEIN_JAR=%LEIN_HOME%leiningen-%LEIN_VERSION%-standalone.jar
if "x%JAVA_HOME%" == "x" (
    set JAVA_HOME=%LEIN_HOME%..\jdk
)
set LEIN_JAVA_CMD=%JAVA_HOME%\bin\java.exe
rem ==================path==================
set Path=%CLJBOX_HOME%clj;%LEIN_HOME%;%R_BIN%;%JRI_DIR%;%Rtools_Home%\bin;%Rtools_Home%\mingw_32\bin;%mvn_repo%;%mvn_home%;%mvn_bin%;%mvn_conf%;%CONEMU_HOME%;%GIT_HOME%;%GIT_BIN%;%GIT_CMD%;%GIT_MINGW32_BIN%;%GIT_USER_BIN%;%Path%
rem ==================echo==================
echo     CLJBOX_HOME=%CLJBOX_HOME%
echo       JAVA_HOME=%JAVA_HOME%
echo   LEIN_JAVA_CMD=%LEIN_JAVA_CMD%
echo        LEIN_JAR=%LEIN_JAR%
echo          R_HOME=%R_HOME%
echo LD_LIBRARY_PATH=%LD_LIBRARY_PATH%
echo          EDITOR=%CLJBOX_HOME%Notepad++\notepad++.exe
echo PATH=%PATH%
rem ======================================
